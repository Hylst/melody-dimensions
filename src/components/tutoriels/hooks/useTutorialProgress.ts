...
export const useTutorialProgress = () => {
   ...
   */
  const isSectionCompleted = useCallback((tutorialId: string, sectionId: string): boolean => {
   ...
   */
  const isTutorialCompleted = useCallback((tutorialId: string, sectionsCount?: number): boolean => {
    // Si le tutoriel est explicitement marqué comme complété
    if (progress[tutorialId]?.isCompleted) return true;
    
    // Si sectionsCount est fourni, vérifier si toutes les sections sont complétées
    if (sectionsCount !== undefined) {
      const completedSections = progress[tutorialId]?.completedSections || [];
      return completedSections.length === sectionsCount;
    }
   ...
   */
  const getTutorialProgress = useCallback((tutorialId: string, totalSections: number): number => {
